<?php
     ...
     * @version             $Id: MPay24Api.php 4614 2011-10-07 10:08:47Z anna $
    ...
    class MPay24Api {
         ...
         */
        private $etp_url         = "https://test.mpay24.com/app/bin/etpproxy_v14";
        /**
        ...
        public function ListPaymentMethods(){
            $xml = $this->buildEnvelope();
             ...
             $operation = $xml->createElementNS('http://www.mpay24.com/soap/etp/1.4/ETP', 'n2:ListPaymentMethods');
        ...
        public function SelectPayment($mdxi){
            $xml = $this->buildEnvelope();
             ...
             $operation = $xml->createElementNS('http://www.mpay24.com/soap/etp/1.4/ETP', 'n2:SelectPayment');
            $operation = $body->appendChild($operation);
        ...
        public function ProfilePayment($requestString){
            $xml = $this->buildEnvelope();
             ...
             $operation = $xml->createElementNS('http://www.mpay24.com/soap/etp/1.4/ETP', 'n2:AcceptPayment');
            $operation = $body->appendChild($operation);
            ...
            $requestNode = $requestXML->getElementsByTagName("request")->item(0);
            
            $requestNode = $xml->importNode($requestNode, true);
            $operation->appendChild($requestNode);
        ...
        public function ExpressCheckoutPayment($requestString){
            $xml = $this->buildEnvelope();
             ...
             $operation = $xml->createElement('n2:AcceptPayment');
            $operation = $body->appendChild($operation);
        ...
        public function CallbackPaypal($requestString){
            $xml = $this->buildEnvelope();
             ...
             $operation = $xml->createElement('n2:ManualCallback');
            $operation = $body->appendChild($operation);
            
            $simpleXmlElement = new SimpleXMLElement($requestString);
            $domElement = dom_import_simplexml($simpleXmlElement);
            $domElement = $xml->importNode($domElement, true);
            $operation->appendChild($domElement);

            $paypalPayment = $xml->getElementsByTagName("callback")->item(0);
            $paypalPayment->setAttributeNS('http://www.w3.org/2001/XMLSchema-instance' ,'xsi:type', 'n2:CallbackPAYPAL');
         ...
         */
        public function LessClear($mPAYTid, $amount, $currency){
            $xml = $this->buildEnvelope();
             $body = $xml->getElementsByTagNameNS('http://schemas.xmlsoap.org/soap/envelope/', 'Body')->item(0);
             
             $operation = $xml->createElementNS('http://www.mpay24.com/soap/etp/1.4/ETP', 'n2:LessClear');
            $operation = $body->appendChild($operation);
            
            $merchantID = $xml->createElement('merchantID', substr($this->merchantid, 1));
            $merchantID = $operation->appendChild($merchantID);
        
            $price = $xml->createElement('price', $amount);
            $price = $operation->appendChild($price);    
            
            $xmlCurrency = $xml->createElement('currency', $currency);
            $xmlCurrency = $operation->appendChild($xmlCurrency);

            $timeStamp = $xml->createElement('timeStamp', date('Y-m-d\TH:i:s', strtotime(date('Y-m-d H:i:s'))));
            $timeStamp = $operation->appendChild($timeStamp);    
        ...
        public function ManualCredit($mPAYTid, $amount, $currency, $customer){
            $xml = $this->buildEnvelope();
             ...
             $operation = $xml->createElementNS('http://www.mpay24.com/soap/etp/1.4/ETP', 'n2:ManualCredit');
            $operation = $body->appendChild($operation);
            
            $merchantID = $xml->createElement('merchantID', substr($this->merchantid, 1));
            $merchantID = $operation->appendChild($merchantID);
        
            $xmlCustomer = $xml->createElement('customer', $customer);
            $xmlCustomer = $operation->appendChild($xmlCustomer);
        ...
        public function ManualReverse($mPAYTid){
            $xml = $this->buildEnvelope();
             ...
             $operation = $xml->createElementNS('http://www.mpay24.com/soap/etp/1.4/ETP', 'n2:ManualReverse');
            $operation = $body->appendChild($operation);
        ...
        public function TransactionStatus($mPAYTid=null, $tid=null){
            $xml = $this->buildEnvelope();
             ...
             $operation = $xml->createElementNS('http://www.mpay24.com/soap/etp/1.4/ETP', 'n2:TransactionStatus');
            $operation = $body->appendChild($operation);
        ...
        private function setSystem($test=null) {
            if($test){
               $this->test = true;
               $this->etp_url = "https://test.mpay24.com/app/bin/etpproxy_v14";
            } else {
                $this->test = false;
                $this->etp_url = "https://www.mpay24.com/app/bin/etpproxy_v14";
            }
        }
        ...
        private function buildEnvelope(){
            ...
            $envelope = $this->soap_xml->createElementNS('http://schemas.xmlsoap.org/soap/envelope/', 'n1:Envelope');
            $envelope->setAttributeNS('http://www.w3.org/2000/xmlns/' ,'xmlns:xsd', 'http://www.w3.org/2001/XMLSchema');
            $envelope->setAttributeNS('http://www.w3.org/2000/xmlns/' ,"xmlns:n2","http://www.mpay24.com/soap/etp/1.4/ETP");
            ...
            $body = $this->soap_xml->createElementNS('http://schemas.xmlsoap.org/soap/envelope/', 'n1:Body');
        ...
        /**
         ...
         private function send() { 
         	$userAgent = 'mPAY24 PHP API $Rev: 4614 $ ($Date:: 2011-10-07 #$)';
            ...
            if($this->debug){
            	$fh = fopen(__DIR__."/curllog.log", 'a+') or $this->dieWithMsg("Can't open file! Please set the needed read/write rights!");
    ...
    class GeneralResponse {
        ...
        function GeneralResponse($response){
        	if($response != ''){
	            $responseAsDOM = DOMDocument::loadXML($response);if(!empty($responseAsDOM) && is_object($responseAsDOM))
    ...
    class PaymentResponse extends GeneralResponse {
        ...
        var $mpayTID;
        /**
         * @var         STRING                            An ID-Number in case of error
         */
        var $errNo;
        /**
        ...
        var $location;
        /**
         * @var         STRING                            The external status from the request, which was sent to mPAY24
         */
        var $externalStatus;
        ...
        function PaymentResponse($response){
        	...
        	if($response != ''){	            
	            $responseAsDOM = DOMDocument::loadXML($response);
	            
	            if(!empty($responseAsDOM) && is_object($responseAsDOM)){        
	                $this->mpayTID             = $responseAsDOM->getElementsByTagName('mpayTID')->item(0)->nodeValue;
	                $this->errNo             = $responseAsDOM->getElementsByTagName('errNo')->item(0)->nodeValue;
	                $this->location            = $responseAsDOM->getElementsByTagName('location')->item(0)->nodeValue;
	                $this->externalStatus    = $responseAsDOM->getElementsByTagName('externalStatus')->item(0)->nodeValue;
	            }
	         } else {
        ...
        /**
         * @return        STRING                            The error number, returned from mPAY24
         */
        ...
        /**
         * @return        STRING                            The external status, returned from mPAY24
         */
