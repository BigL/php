<?php
     ...
     * @version             $Id: MPay24Shop.php 4614 2011-10-07 10:08:47Z anna $
    ...
    abstract class MPay24Shop extends Transaction {
        ...
        /**
         * @abstract                                    Actualize the transaction, which has a transaction ID = $tid with the values from $args
        ...
        function pay(){
            ...
            if(!$mdxi || !$mdxi instanceof ORDERXML)
        ...
        function payWithProfile(){
            ...
            if(!$order || !$order instanceof ORDERXML)
        ...
        function payWithExpressCheckout(){
            ...
            $order->request->timeStamp = date('Y-m-d\TH:i:s', strtotime(date('Y-m-d H:i:s')));
            $order->request->merchantID = $this->mPay24Api->getMerchantID();
            $order->request->pType = "PAYPAL";
            $order->request->payment->commit = "false";
        ...
        function finishExpressCheckoutPayment($tid, $shippingCosts, $amount, $cancel){
            ...
            $this->write_log("FinishExpressCheckout",
                            "REQUEST to " . $this->mPay24Api->getEtpURL() . " - ".str_replace("><", ">\n<", $this->mPay24Api->getRequest())."\n");
            $this->write_log("FinishExpressCheckout",
                            "RESPONSE - ".str_replace("><", ">\n<", $this->mPay24Api->getResponse())."\n");
        ...
        function confirm($tid, $args){
            ...
            $this->write_log("Confirmation for transaction " . $tid . ":", utf8_encode($to_log)."\n");                
            ...
            if($transactionStatus->getParam("SHIPPING_ADDR"))
                $order = DOMDocument::loadXML($transactionStatus->getParam("SHIPPING_ADDR"));
            if(isset($order)){
         ...
         * @return        ARRAY                            The transaction status of a payment. Possible values:
         *                             
        ...
        function updateTransactionStatus($tid){
            ...
            if(!$transaction->MPAYTID || !is_numeric($transaction->MPAYTID)){
                ...
                if($tidTransactionStatusResult->getParam("SHIPPING_ADDR"))
                    $order = DOMDocument::loadXML($tidTransactionStatusResult->getParam("SHIPPING_ADDR"));
                ...
                if($mPAYTidTransactionStatusResult->getParam("SHIPPING_ADDR"))
                    $order = DOMDocument::loadXML($mPAYTidTransactionStatusResult->getParam("SHIPPING_ADDR"));
        ...
        function clearAmount($tid, $amount){
            ...
            $clearAmountResult = $this->mPay24Api->LessClear($mPAYTid, $amount, $currency);
