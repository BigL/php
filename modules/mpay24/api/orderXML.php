 ...
 * @author            support@mpay24.com
     * @version             $Id: orderXML.php 4385 2011-05-31 06:55:57Z anna $
...
class ORDERXML
{
     ...
     */
    public function ORDERXML($doc=null,$node=null) {
        if ($doc)
        $this->doc = $doc;
        else {
    ...
    public function __call($method, $args) {
        if(substr($method, 0, 3) == "set" && $args[0] != '')
        {
            ...
            $this->node->setAttribute($attributeName, $args[0]);
        }
        elseif($args[0] != '')
        {
            if(sizeof($args)>2)
            ...
            if ($qry->length > 0)
            {
                return new ORDERXML($this->doc,$qry->item(0));
            }
            else
            {
                if(array_key_exists(1, $args))
                $node = $this->doc->createElement($method, $args[1]);
                else
                $node = $this->doc->createElement($method);
                $node = $this->node->appendChild($node);
                return new ORDERXML($this->doc,$node);
            }
        }
    }
