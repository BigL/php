...
class PayPal extends PaymentModule
{
	...
	public function __construct()
	{
		...
		$this->version = '2.8.7';
		$this->module_key = '646dcec2b7ca20c4e9a5aebbbad98d7e';
		...
		/* Init Current Index */
		$this->currentIndex = 'index.php?controller='.htmlentities(Tools::getValue('controller')).'&tab='.htmlentities(Tools::getValue('tab')).'&configure='.htmlentities(Tools::getValue('configure')).'&token='.htmlentities(Tools::getValue('token')).'&tab_module='.htmlentities(Tools::getValue('tab_module')).'&module_name='.htmlentities(Tools::getValue('module_name')).'';
	...
	public function hookPaymentReturn($params)
	{
		...
		# return $this->display(_PS_THEME_DIR_, 'order-confirmation.tpl');
	}

	public function hookRightColumn($params)
	{
		$this->context->smarty->assign('iso_code', Tools::strtolower($this->context->language->iso_code));
			...
			else
			{
				if ($response['ACK'] == 'Success')
				{
					if ($response['PAYMENTSTATUS'] == 'Completed' OR $response['PAYMENTSTATUS'] == 'Reversed' OR ($response['PAYMENTSTATUS'] == 'Pending' AND $response['PENDINGREASON'] == 'authorization'))
						Tools::redirectAdmin($this->currentIndex.'&id_order='.(int)(Tools::getValue('id_order')).'&vieworder&paypal=validationOk&token='.Tools::getAdminToken('AdminOrders'.(int)(Tab::getIdFromClassName('AdminOrders')).(int)$this->context->employee->id));
					else
			...
			else
			{
				if ($response['ACK'] == 'Success')
				{
					if ($response['PAYMENTSTATUS'] == 'Completed')
						Tools::redirectAdmin($this->currentIndex.'&id_order='.(int)(Tools::getValue('id_order')).'&vieworder&paypal=captureOk&token='.Tools::getAdminToken('AdminOrders'.(int)(Tab::getIdFromClassName('AdminOrders')).(int)$this->context->employee->id));
					else
						Tools::redirectAdmin($this->currentIndex.'&id_order='.(int)(Tools::getValue('id_order')).'&vieworder&paypal=captureError&token='.Tools::getAdminToken('AdminOrders'.(int)(Tab::getIdFromClassName('AdminOrders')).(int)$this->context->employee->id));
				}
				else
					$this->_html .= '<p style="color:red;">'.$this->l('Error from PayPal: ').$response['L_LONGMESSAGE0'].' (#'.$response['L_ERRORCODE0'].')</p>';
			}
			...
			else
			{
				if ($response['ACK'] == 'Success')
				{
					if ($response['REFUNDTRANSACTIONID'] != '')
						Tools::redirectAdmin($this->currentIndex.'&id_order='.(int)(Tools::getValue('id_order')).'&vieworder&paypal=refundOk&token='.Tools::getAdminToken('AdminOrders'.(int)(Tab::getIdFromClassName('AdminOrders')).(int)$this->context->employee->id));
					else
						Tools::redirectAdmin($this->currentIndex.'&id_order='.(int)(Tools::getValue('id_order')).'&vieworder&paypal=refundError&token='.Tools::getAdminToken('AdminOrders'.(int)(Tab::getIdFromClassName('AdminOrders')).(int)$this->context->employee->id));
				}
				else
					$this->_html .= '<p style="color:red;">'.$this->l('Error from PayPal: ').$response['L_LONGMESSAGE0'].' (#'.$response['L_ERRORCODE0'].')</p>';
			}
