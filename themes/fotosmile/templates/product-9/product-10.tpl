...
var product_template = "product-`$product->id`.tpl"
var product_folder = "product-`$product->id`"
	...
	<div id="pb-left-column">
		...
		<form id="buy_block" {if $PS_CATALOG_MODE AND !isset($groups) AND $product->quantity > 0}class="hidden"{/if} action="{$link->getPageLink('cart')}" method="post">
			...
			<p class="hidden">
				...
				<input type="hidden" name="print_json" id="print_json" value="1" />
		...
		<div class="content_prices clearfix">
			...
			{else}
				...
				<p id="add_to_cart" class="buttons_bottom_block">
					<span></span>
					<a onClick="doSave();"  id="button_order_cart" class="exclusive{if $order_process == 'order-opc'}_large{/if}" title="{l s='Check out' mod='blockcart'}"><span></span>{l s='Check out' mod='blockcart'}</a>
				</p>
				{else}
					<span> 
						{l s='Log In' mod='authsocialps'}
					</span>
				{/if}
			{/if}
...
<script type="text/javascript">
    function doSave() {
    	$('#print_json').val(JSON.stringify({$print_json}));
